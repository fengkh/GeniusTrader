...
- 公告候选不直接触发 AI 分析。只有用户导入为 `source_type=announcement` 的 InformationItem 后，才复用既有 `/information/{item_id}/analyze`。
- `metadata_only` 导入只代表公告元数据可用，AI 必须说明正文未提取，不能补写公告正文。
- `provider_document` 表示用户按需提取的公告 PDF 文本层，仍需显示提取局限，不代表表格、扫描件或版式信息完整。
- `user_supplemented` 表示用户补充文本，AI 必须将其与来源公告元数据区分。
- AI 不得生成公告中不存在的金额、日期、主体、股票、行情、财务、估值、价格数字或来源时间。
- AI 不得把标题分类当作完整事实，不得自动判断利好利空，不得输出买卖、仓位、目标价或收益预测。
- 来源等级仅用于来源分层，不得单独决定情绪方向、事实强度或投资影响。
