...
| AC-21 | 站内通知中心 | 当前真实支持用户每日复盘、重要信息、待核实事项和 AI 失败事件的站内通知；支持未读数、事件筛选、严重度、已读/未读/归档和目标页面跳转；公告、异动、观察条件、估值和系统事件为未来事件来源 | 通知缺少目标页面、跨用户展示，或通知失败导致复盘和行情不可用 |
...
| AC-48 | 通知偏好和通道边界 | `/settings/notifications` 的站内通知偏好接真实 API，支持开启/关闭、immediate/daily_digest/disabled、最低严重度和免打扰时间；微信公众号区域仅展示 Mock 说明并明确未接入真实微信能力 | 页面发起真实 OAuth、扫码、SDK、模板消息或外部 API 调用，或把微信偏好保存为真实通道 |
...
- `daily_reviews`、`daily_review_versions`、`daily_review_items`、`business_events`、`notifications`、`notification_preferences` 和 `notification_deliveries` 迁移成功，ORM 与迁移约束一致。
- 用户可在 `/reviews` 按日期和状态查看真实用户每日复盘列表，并生成今日或历史日期复盘。
- 无当日信息时可生成 `empty` 复盘；存在未分析、分析失败、内容不足或未确认关系时生成 `partial`。
- AI 未配置时生成 `rules_only` 规则复盘；AI 失败时保存 `rules_with_ai_fallback`，规则快照不丢失。
- 复盘只使用当前用户、同 `review_date`、未归档且有可用正文的信息；业务日期优先使用 `published_at`，缺失时使用 `created_at`。
...
- 复盘生成、部分完成、stale、重要信息、待核实事项和 AI 失败会产生 BusinessEvent；severity 由程序规则决定，AI 不得提升。
- 真实站内 Notification 支持列表、未读数、标记已读、标记未读、归档、批量已读和 deep_link 跳转。
- `notification_preferences` 当前仅支持 `in_app`；`daily_digest` 当前不产生单条即时站内通知，且不代表已实现定时摘要发送。
- `/settings/notifications` 站内通知偏好读写真实 API；微信公众号区域保持“尚未接入”说明，不创建 OAuth、OpenID、二维码、模板消息或微信 API 调用。
- 用户 A 不能读取、重新生成、查看版本、标记通知或修改用户 B 的复盘、通知和通知偏好。
- 自动测试 Mock 全部 AI 调用，不调用用户真实 Provider；日志和审计不保存完整正文、完整 Prompt、API Key、Session Token、Cookie 或数据库密码。

## 第五阶段公告与资讯数据源 Spike 验收项

- AC-57：公告与资讯 Provider Spike 必须在隔离目录内运行，输出目录被 Git 忽略，不写入正式业务数据库，不创建迁移、调度、队列或生产 Provider。失败示例：Spike 结果直接写入 `information_items` 或把候选来源注册为生产服务。
- AC-58：Spike 报告必须区分技术可达、字段覆盖、授权/留存/再展示未确认三类结论，且不得把 CNINFO、交易所、监管网站或任何资讯源写成已冻结正式供应商。失败示例：文档声称“公告供应商已确定为 CNINFO”。
