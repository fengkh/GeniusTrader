...
- 每日复盘 `rule_snapshot` 新增纳入当前用户显式创建或更新的研究事项、观察条件和观察验证结果。AI 仍只能解释服务端快照，不得创建、覆盖或关闭研究事项。
- 信息分析和每日复盘可输出建议研究事项和建议观察条件，但建议只保存在对应分析结果或复盘版本中；用户点击采纳后才写入 `research_tasks`。
- 复盘建议采纳必须使用当前用户、来源复盘版本和建议标识做幂等，不得重复创建事项，也不得因为复盘生成成功自动采纳建议。
- 新增业务事件 `research_task.created`、`research_task.status_changed`、`research_task.due` 和 `observation_condition.due`。研究事项创建和状态变更默认不产生即时站内通知；到期观察条件可产生站内提醒。
- 观察条件验证结果可以进入下一次复盘材料，但不得表达为买入、卖出、仓位、目标价、收益预测或保证性结论。
- 无真实行情或行情 Provider 关闭时，复盘和研究工作台继续使用公告、信息、研究事项和用户输入；AI 不得补造行情数字。
