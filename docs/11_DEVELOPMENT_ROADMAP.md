...
## 2026-07-28 第八阶段：模块化自选股研究工作台 V0.2

目标：

- 将今日页升级为研究工作台首页，聚合当前用户的待办、重点股票、研究事项、观察条件和最新复盘状态。
- 将自选股页升级为扫描器，在不依赖真实行情的情况下按公告、信息、研究事项、观察条件和复盘 stale 状态计算关注分。
- 将个股详情升级为个股研究档案，聚合用户关注逻辑、官方信息、研究事项、研究时间线和历史复盘。
- 在信息中心下新增 `/information/tasks` 二级页面，支持研究事项创建、筛选、状态更新和历史更新。
- 将 AI 分析和每日复盘建议限定为固定 Schema 建议，用户显式采纳后才创建研究事项或观察事项。

限制：

- 不新增 AI 聊天、自由问答、AI 助手一级导航、自然语言选股、交易建议、实时行情、分时图、五档盘口、估值中心、微信、支付、Docker 部署或真实 Provider 联网 Smoke。
- 无经授权真实行情时继续显示 `unavailable`，不得回退 Mock 行情或用 AI 补造价格、成交量、财务、公告正文或事实。
- 研究事项、观察条件和采纳操作必须按当前 `user_id` 隔离，普通用户 API 不接受客户端提交 `user_id`。

输出：

- 新增 `docs/28_MODULAR_RESEARCH_WORKBENCH_V02_DRAFT.md`。
