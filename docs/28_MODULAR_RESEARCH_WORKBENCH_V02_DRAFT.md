...
- `/today/overview` 在原有研究总览基础上增加当前用户自选股的行情交易日、快照覆盖数量、暂无行情数量和涨跌分布。
- `/watchlist/scanner` 增加基于已入库日级快照的上涨、下跌、有行情、无行情筛选，以及涨跌幅、成交额、换手率排序。
- `/stocks/{stock_id}/research-dossier` 在股票身份之后展示真实日级行情快照区，列出来源、交易日、收盘价、涨跌幅、成交量、成交额、换手率、市值、PE/PB、完整度、新鲜度和缺失字段。
- 这些工作台 API 仍然只是程序聚合读模型：不触发同步、不调用 AI、不生成复盘、不修改研究事项状态。
- `partial` 快照不得被归为无行情；它表示已有部分快照字段，页面必须展示缺失字段。
- 无行情时继续展示公告、信息、研究事项、观察条件和复盘状态，不回退到 Mock 或 0 值。
- AI 分析和每日复盘不得生成、补全或覆盖真实日级行情字段。
