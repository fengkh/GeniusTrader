...
## 2026-07-28 第七阶段 Checkpoint A 衔接

证券主数据继续作为行情、公告和自选股的股票身份基础。新建行情同步只能引用已有 `stocks`，不得通过行情 Provider 创建股票；公告 Provider 也只能匹配已有 `stock.id`，不得为 BJ 新旧代码或未知代码创建重复股票。

`/watchlist`、`/today` 和 `/watchlist/[stockId]` 可以展示真实日级行情快照状态；没有真实快照时必须展示 unavailable，不得把证券主数据、开发种子或 Mock 数据展示成行情。
